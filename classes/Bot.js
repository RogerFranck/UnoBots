...
export default class Bot extends Component {
  ...
  getOptionPlayCard = (cardInPlayZone) => {
    ...
    return this.state.hand.filter(
      (e) =>
        e.color == cardInPlayZone.color || e.number == cardInPlayZone.number
    );
  };
  ...
  getColor = (ally) => {
    ...
    const colorCount = target.show().reduce(
      (acc, card) => {
    ...
    const selectedColor = Object.entries(colorCount).reduce(
      (acc, [key, value]) => {
  ...
  play = (ally, turn) => {
    ...
    if (this.isOptions(options)) {
      ...
      const betterCard = this.props.isIaTeam
        ? this.suggest(ally, options, turn)
        : this.selectedCard(options); //? Dependiendo del modo del robot: 'individualista' o 'coperativo selecciona una carta
      if (betterCard === "draw") {
        //? Si lo mejor para la victoria es tomar una carta lo hace
        this.draw();
        return [this.state.name, "x", "Especial"];
      }
      this.playCard(betterCard);
      if (betterCard.color === "Especial") {
        //? En caso de jugar una especial determinar el mejor color a tomar
        this.getColor(ally);
      }
      logMessage = [this.state.name, betterCard.number, betterCard.color];
    } else {
      this.draw(); //? Si no puede jugar nada entonces toma una carta
      logMessage = [this.state.name, "x", "Especial"];
    }
    return logMessage;
  };
