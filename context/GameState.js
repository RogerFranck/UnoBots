...
const GameState = ({ children }) => {
  ...
  const OpenSelectedCardModal = (value) => {
    //* Controlador para abrir modal de selección de color [para humanos]
    dispatch({
  ...
  const setEffectsSounds = (sonido, target) => {
    //* Controlador de carga de sonidos
    dispatch({
      type: SET_SOUND,
      payload: {
        sonido,
  ...
  const NextPlayer = (skip) => {
    //* determina el siguiente jugador, si recibe skip es por que se proceso en otro lugar el nuevo jugador
    let newPlayer = state.turno + state.direction; //* direction es '1' o '-1' y funciona para conocer la sequeincia del juego
    if (newPlayer > 2) {
      //* los jugadores son 0:Humano, 1:Deimos, 2:Fobos, siempre
      newPlayer = 0;
    } else if (newPlayer < 0) {
      newPlayer = 2;
    }
  ...
  const skipPlayCard = () => {
    //* Ejecuta el cambio de turno saltandose a un jugador cuando se juega la carta "a" o "skip"
  ...
  const reversePlayCard = () => {
    //* Cambia la sequiencia de juego cuando se juega la carta "b" o "Reverse"
  ...
  const drawFourPlayCard = (skip) => {
    //* Cuando se juega un +4 salta turno, elije un color [humano] y toma 4 cartas
    ...
    if (currentPlayer == 2) {
      //* next player local para no afectar el estado
      newPlayer = 0;
    } else {
      newPlayer = currentPlayer + 1;
    }
    const handList = [
      ...state[state.players[newPlayer]],
      cardDraw,
      cardDraw2,
      cardDraw3,
      cardDraw4,
    ]; //* Las mete en el siguiente jugador
    dispatch({
  ...
  const drawTwoPlayCard = (skip) => {
    //* Cuando se juega un +2 salta de turno y el target enemigo toma 2 cartas
    const stackList = state.Stack;
    const cardDraw = stackList.pop();
    const cardDraw2 = stackList.pop();
    const currentPlayer = state.turno;
  ...
  const changeColorEspecialCard = (color) => {
    //* Al jugar una wildCard especial te permite cambiar su color
    const playZoneCard = state.PlayZone[state.PlayZone.length - 1];
    let skip = false;
    if (playZoneCard.number == "d") {
      //* de ser un +4 entonces realiza un cambio de turno adicional
      skip = skipPlayCard();
    }
  ...
  const drawTwoCardUnoButton = (skip) => {
    //* Castigo por no presionar el uno btn a tiempo - toma 2 cartas
    const stackList = state.Stack;
    const cardDraw = stackList.pop();
    const cardDraw2 = stackList.pop();
  ...
  const wildPlayCard = () => {
    //* abre el modal de selección de color - [solo para humanos]
    OpenSelectedCardModal(true);
  };

  const setUpGame = (reload = false, data = {}) => {
    //* Carga inicial del juego - asigna las manos, stack y primera carta del playZone
    if (!reload) {
      const deck = new Deck();
      const { playerHand, FobosBot, DeimosBot } = drawHands(deck);
      dispatch({
  ...
  const PlayPlayerCards = (card, target) => {
    //* Controlador de jugar carta [humanos y robots]
    if (
      card.color == state.PlayZone[state.PlayZone.length - 1].color ||
      ...
      switch (
        card.number //* por cada carta especial ejecuta su función
      ) {
        case "a":
          skip = skipPlayCard();
          break;
...
const drawHands = (deck) => {
  //* Reparte 7 cartas a cada jugador usando la clase deck
  const hands = {
    playerHand: [],
    FobosBot: [],
    DeimosBot: [],
